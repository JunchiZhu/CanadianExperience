...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnMachineSelection, this, XRCID("EditMachineSelection"));
    ...
    parent->Bind(wxEVT_UPDATE_UI, &ViewEdit::OnUpdateEditMachineSelection, this, XRCID("EditMachineSelection"));
...
/**
 * Handle an Edit>MachineSelection menu option
 * @param event The event we update
 */
void ViewEdit::OnMachineSelection(wxCommandEvent& event)
{
    //ShowDigBox(this->GetParent());
}

/**
 * Update the user interface for Edit>MachineSelection
 * @param event The event we update
 */
void ViewEdit::OnUpdateEditMachineSelection(wxUpdateUIEvent& event)
{
    //event.Check(mMode == Mode::Rotate);
}
